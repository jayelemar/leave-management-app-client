  ...
  return (
    <details>
      <summary
        ...
        onClick={toggleMenu}
      >
        <div className="flex justify-start items-center flex-row gap-2 sideitem  w-[100%] py-2 px-2  cursor-pointer text-lg">
          {item.icon}
          {item.title}
        </div>
      </summary>
      <ul>
        { (item.children as ItemProps[]).map((child) => (
          <li 
            key={child.path}
            className="sideitem w-[97%] py-2 px-0  cursor-pointer text-lg flex gap-2 my-2  "
          >
            <NavLink 
            to={child.path ?? '/'} 
